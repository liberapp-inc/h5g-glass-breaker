...
var GeneratePlate2 = (function (_super) {
    __extends(GeneratePlate2, _super);
    ...
    GeneratePlate2.prototype.generateGlassPlate = function () {
        // ガラスの位置や移動方向を決定
        this.decideProperty();
     ...
     * Decide the direction and position of the glasses
     */
    GeneratePlate2.prototype.decideProperty = function () {
        //出現するプレートの種類の決定
        this.glassPlateType = 0 + Math.floor(Math.random() * 2); //0~1
        //各プレートの描画とクリック判定の付与
        switch (this.glassPlateType) {
     ...
     * Move glasses
     */
    GeneratePlate2.prototype.moveGlassPlate = function () {
        if (this.glassPlateMoveFlag == true && Main.stageLevel != Stage.GAME_OVER) {
            //ガラスの出現位置の決定
            switch (this.glassPlateImagePosition) {
                case GlassPosition.UP:
     ...
     * ガラスの動きを止める
     */
    GeneratePlate2.prototype.moveStop = function () {
        this.glassPlateMoveFlag = false; //Enabled move
        this.glassPlateImage.touchEnabled = false; //Enabled touch
        return false;
    };
     ...
     * TouchEvent
     */
    GeneratePlate2.prototype.glassTouch = function (evt) {
        this.moveStop();
        switch (this.glassPlateType) {
            case GlassPlateType.GLASS:
                //ガラスが割れる音
    ...
    GeneratePlate2.prototype.fadeBrokenGlass = function () {
        if (this.fadeFlag == false) {
            this.fadeTime = egret.getTimer();
            egret.startTick(this.fadeMethod, this);
            this.fadeFlag = true;
     ...
     * Make the broked glass transparent
     */
    GeneratePlate2.prototype.fadeMethod = function () {
        if (this.fadeFlag == true) {
            this.glassPlateImage.alpha -= this.fadeSpeed;
            if (this.glassPlateImage.alpha <= 0) {
                this.glassPlateImage.alpha = 0;
     ...
     * Calculate score
     */
    GeneratePlate2.prototype.calculateScore = function () {
        GeneratePlate.breakComboNumber += 1;
        switch (GeneratePlate.breakComboNumber) {
            case 0:
                GeneratePlate.breakComboBonus = 1;
    ...
    GeneratePlate2.glassPlateMoveSpeedMagnification = 1;
    GeneratePlate2.glassBreakNumber = 0; //ガラスを破壊した数
    GeneratePlate2.breakComboNumber = 0; //コンボ数
    GeneratePlate2.breakComboBonus = 1;
    GeneratePlate2.score = 0;
    return GeneratePlate2;
}(GameObject));
__reflect(GeneratePlate2.prototype, "GeneratePlate2");
